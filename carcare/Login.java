...
public class Login extends Usuario {
	...
	public void criaLogin() {		// CHAMADO PELO MAIN ELE IRÁ CRIAR NOVOS LOGINS UNICOS QUE NAO PODEM SER REPETIDOS
		...
		for (int y = 0; y < ArrayLogin.size(); y++) {					// O "y" CAMINHARÁ ATÉ O TAMANHO TOTAL DO ARRAYLIST
			if (getUsuario().equals(ArrayLogin.get(y).getUsuario())) {	// SE ENTRAR O VALOR QUE VOCE DIGITOU ELE DA A MENSAGEM DE USUARIO EM USO
				System.out.println("Usuario em uso!");
				break;													// AQUI ELE FORÇA PARA SAIR DO LAÇO "for"
			} else if (y == ArrayLogin.size() - 1) {					// SE ELE NAO ENCONTRAR NENHUM VALOR IGUAL NO ARRAY ENTAO ELE AVANÇA
				System.out.println("Adicionado com sucesso!");
				ArrayLogin.add(new Login(getUsuario(), getSenha()));	// AQUI ELE PASSA OS PARAMETROS PARA CRIAÇAO NA CLASSE "login" METODO "criaLogin"
				break;													// AQUI ELE FORÇA PARA SAIR DO LAÇO "for"
			}
		}
	}
	...
	public void fazLogin() {		// FAZ O LOGIN COM USUARIO E SENHA
		do {
			...
			for (int y = 0; y < ArrayLogin.size(); y++) {					// O "y" CAMINHARÁ ATÉ O TAMANHO TOTAL DO ARRAYLIST
				if (getUsuario().equals(ArrayLogin.get(y).getUsuario())) {	// SE O QUE EU DIGITEI FOR IGUAL A UMA POSICAO DO ARRAY ELE AVANÇA
					if (getSenha().equals(ArrayLogin.get(y).getSenha())) {	// SE O QUE EU DIGITEI EM SENHA FOR IGUAL A MESMA POSÇAO DO "Y" ELE AVANÇA
						System.out.println("Bem vindo " + ArrayLogin.get(y).getUsuario() + "!");	// MSG DE BEM VINDO
						cont = 1;											// CONT SERVE PARA FICAR RODANDO INFINITAMENTE, ENTAO AQUE DEMOS UM VALOR PARA ELE SAIR DO LAÇO WHILE
						codVerificador = y+1;								// O "CodVerificador" É MUITO IMPORTANTE PORQUE ELE IRÁ PASSAR A O CODIGO DO MEU USUARIO PARA A CLASSE "Carro"
						break;						
					} else {
						System.out.println("Senha incorreta!");				// SE ELE NAO ENTRAR NO "if" DA SENHA INCORRETA
						break;
					}
				} else if (y == ArrayLogin.size() - 1) {					// SE O QUE EU DIGITE NAO EXISTE EM NENHUMA POSICAO DO ARRAY ENTAO ELE DA A MSG DE USUARIO OU SENHA ERRADO
	...
	public void listaLogin() {				// MOSTRA TODOS OS LOGINS E SENHAS EXISTENTES PARA FACILITAR NA CRIAÇAO DO CODIGO
		System.out.println("\n----Lista de Logins----\n"); 	
	...
	public Login(String usuario, String senha) {		// CONSTRUTOR COM PARAMETROS USUARIO E SENHAS PARA SEREM INSERIDOS NO ARRAYLIST
